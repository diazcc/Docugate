<template lang="">
    <section class="table-user">
        <nav class="table-user__nav-options">
            <select v-model="selectedFilter" class="table-user__nav-options__select">
        ...
        <div class="flex-table" >
            <div class="flex-table__header">
                <div class="flex-table__header__cell">{{$t('identification_number')}}</div>
                ...
                <div class="flex-table__header__cell">{{$t('date_of_birth')}}</div>
                <div class="flex-table__header__cell">{{$t('dependence')}}</div>
                <div class="flex-table__header__cell">{{$t('role')}}</div>
                                <div class="flex-table__header__cell">{{$t('is_staff')}}</div>

                <div class="flex-table__header__cell">{{$t('state')}}</div>
            </div>
            <div class="flex-table__row " v-if="dataTableUser.data.length<1">
            ...
            <div v-else class="flex-table__row" v-for="data in dataTableUser.data" :key="data.id">
                <div class="flex-table__row__cell">{{ data?.identification_number }}</div>
                <div class="flex-table__row__cell">{{ data?.first_name }}</div>
                <div class="flex-table__row__cell">{{ data?.last_name }}</div>
                <div class="flex-table__row__cell">{{ data?.email }}</div>
                <div class="flex-table__row__cell">{{ data?.date_of_birth }}</div>
                <div class="flex-table__row__cell">{{ data?.dependence ? data.dependence.name : 'N/A' }}</div>
             
                <div class="flex-table__row__cell">{{ data?.role ? data.role.name : 'N/A' }}</div>
                                <div class="flex-table__row__cell">{{ data?.is_staff }}</div>

                <div class="flex-table__row__cell">{{ data?.is_active ? "Active" : 'Inactive'}}</div>
...
const filteredData = computed(() => {
  if (!selectedFilter.value || selectedFilter.value == "all") {
    return props.dataTableUser.data;
  } else {
    return props.dataTableUser.data.filter((item: any) => {
